      ...
      <DashboardLayout title="Dashboard">
        <div className="flex items-center justify-center h-64">
          <div className="flex flex-col items-center gap-4">
            <div className="animate-spin rounded-full h-12 w-12 border-4 border-primary-100 border-t-accent-500"/>
            <p className="text-primary-400 text-sm animate-pulse">Loading dashboard...</p>
          </div>
        </div>
      </DashboardLayout>
    )
  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div className="relative overflow-hidden rounded-2xl mb-8 p-6"
        style={{ background: 'linear-gradient(135deg, #0a1628 0%, #1e3a5f 40%, #2d1f0e 100%)' }}>
        <div className="absolute top-0 right-0 w-64 h-64 rounded-full opacity-[0.03] bg-white transform translate-x-16 -translate-y-16"/>
        <div className="absolute bottom-0 left-1/3 w-40 h-40 rounded-full opacity-[0.02] bg-white transform translate-y-10"/>
        <div className="absolute top-4 right-8 w-20 h-20 rounded-full opacity-[0.04] bg-accent-400 blur-2xl"/>
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <p className="text-accent-200 text-sm font-medium">{greeting()},</p>
            <h2 className="text-2xl font-bold text-white mt-1">{user?.email?.split('@')[0]}</h2>
            <p className="text-accent-200/70 text-sm mt-1 capitalize">{user?.role?.replace('_', ' ')}</p>
          </div>
          <div className="text-right">
            <p className="text-white text-3xl font-bold tabular-nums">
              {time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
            </p>
            <p className="text-accent-200/70 text-sm mt-1">
      ...
      <h3 className="text-lg font-bold text-primary-700 mb-4">Alerts</h3>
      ...
      {/* Quick Actions */}
      <h3 className="text-lg font-bold text-primary-700 mb-4">Quick Actions</h3>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'New Sale', path: '/sales', color: 'from-emerald-600 to-emerald-500' },
          { label: 'New Prescription', path: '/prescriptions', color: 'from-rose-600 to-rose-500' },
          { label: 'Add Drug', path: '/drugs', color: 'from-accent-600 to-accent-500' },
          { label: 'New Order', path: '/purchase-orders', color: 'from-primary-600 to-primary-500' },
        ].map(action => (
